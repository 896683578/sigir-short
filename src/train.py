...
parser.add_argument('--lambda_ctr',type=float, default = 1.0, help = 'scalar hyperparameter to control ctr loss')
parser.add_argument('--lambda_sim',type=float, default = 1.0, help = 'scalar hyperparameter to control sim loss')
parser.add_argument('--T',type=float, default =0.5, help = 'threshold T in similar Matrix')
parser.add_argument('--tau',type=float, default =0.07, help = 'temperature coefficient')
...
def construct_W_p(un_logits,un_logits_org,T=0.25):
    # (B,labels)
    W_p = np.dot(un_logits,un_logits_org.T)
    np.fill_diagonal(W_p,1)
    W_p[W_p < T] = 0 
    return W_p

def construct_W_e(un_e, un_e_org,tau=0.07):
    bsz = un_e.shape[0]
    W_e = np.zeros(bsz,bsz)
    for i in range(bsz):
        for j in range(bsz):
            if i == j:
                W_e[i,j] = np.exp(np.dot(un_e[i],un_e_org[i])/tau)
            else:
                W_e[i,j] = np.exp(np.dot(un_e_org[i],un_e_org[j]/tau))
    return W_e
# ------------------------training----------------------------
...
def main(argv=None):
    ...
    if args.use_aug:
        ...
        unlabeled_dataset_total = pre_processing(sentence_labeled_aug, sentence_labeled_aug_labels,mode='unlabeled',sentence_org=sentence_train,sentence_org_label=sentence_train_label)
    ...
    for epoch in range(0, args.total_epochs):
        ...
        train_loss = 0
        for step, batch in enumerate(labeled_dataloader):
            batch_unlabeled = unlabeled_dataloader_list[epoch][step]
            if step % 40 == 0 and not step == 0:
                # Calculate elapsed time in minutes.
                elapsed = format_time(time.time() - t0)
                # Report progress.
            ...
            W_p = construct_W_p(un_logits,un_logits_org)
            W_e = construct_W_e(un_e,un_e_org,args.tau)
            bsz = un_e.shape[0]
            loss_ctr = 0
            for i in range(bsz):
                exp_sum = 0
                for j in range(bsz):
                    exp_sum += np.exp(np.dot(un_e_org[i],un_e_org[j])/args.tau)
                loss_ctr += -1 * np.log(np.exp(np.dot(un_e_org[i],un_e[i])/args.tau))
            train_loss += args.lambda_ctr *loss_ctr 

            W_p = W_p/np.sum(W_p,axis=1,keepdims=True)
            W_e = W_e/np.sum(W_e,axis=1,keepdims=True)
            # Perform a backward pass to calculate the gradients.
            # loss.sum().backward()
            # optimizer.step()
            ...
            for i in range(bsz):
                ...
                for j in range(bsz):
                    if i == j:
                        loss_sim += -1 * W_e[i,j] *np.log(np.exp(np.dot(un_e_org[i],un_e[i])/args.tau)/ exp_sum) 
                    else:
                        loss_sim += -1 * W_e[i,j] *np.log(np.exp(np.dot(un_e_org[i],un_e_org[j])/args.tau)/ exp_sum) 
            train_loss += args.lambda_sim * loss_sim 
            train_loss.backward()
            optimizer.step()
            scheduler.step()
            # Update the learning rate.
